...
if __name__ == '__main__':
    ...
    model_path = os.path.join(PATH, 'checkpoint/pixelwise-catboost/')
    ...
    n_splits = 14
    model_list = [f'{model_path}pixelwise-catboost-{n}.cbm' for n in range(n_splits)]
    ...
    subs[crop_names] = gmean(oofs, axis=0)
    subs = subs.groupby('field_id').agg(gmean).reset_index()
    subs.to_csv(f'{output_path}/pixelwise-catboost.csv', index=False)
