...
if __name__ == '__main__':
    ...
    model_path = os.path.join(PATH, 'checkpoint/fieldwise-catboost/')
    ...
    def nan_free(f):
      def nan_free_fct(l):
        ll = [x for x in l if not np.isnan(x)]
        if len(ll) == 0:
          return np.nan
        return f(ll)
      return nan_free_fct
    ...
    def agg_data():
      comb_gp = comb.groupby(['field_id'])

      data = []
      fncts = ['mean']
      fs = [np.mean]

      for f, fn in zip(fs, fncts):
        fn_comb = comb_gp.agg(f)
        col_dict = {c: f'{c}_{fn}' for c in fn_comb.columns}
        fn_comb = fn_comb.rename(columns=col_dict)
        data.append(fn_comb)
    ...
    comb = test_data.copy()
    comb = agg_data()
    band_columns = comb.columns.difference(['field_id'], sort=False).values
    comb[band_columns] = comb[band_columns]**0.5

    col_cats = [c for c in band_columns if 'B01' in c]

    vegs = paralellize(vegetation_index, [i for i,_ in enumerate(col_cats)], size=len(col_cats), verbose=1, with_tqdm=True)

    vi_df = pd.DataFrame()
    for i, c in enumerate(col_cats):
      c = c.split('_')[-1]
    ...
    test_data_full = test_data_full.replace([np.inf, -np.inf], np.nan)

    main_cols = test_data_full.columns.difference(['field_id', 'crop_id', 'fold', 'target', 'MMSR_mean', 'NBR_mean', 'ACI_mean'], sort=False).tolist()

    n_splits=14
    model_list = [f'{model_path}fieldwise-catboost-{n}.cbm' for n in range(n_splits)]
    ...
    submission['field_id'] = test_data_full['field_id']

    for i, label in enumerate(agr):
        submission[label] = np.array(predic)[:,i]

    submission.to_csv(f'{output_path}/fieldwise-catboost.csv', index=False)
